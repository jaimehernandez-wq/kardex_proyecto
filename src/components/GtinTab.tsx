...
export function GtinTab({ movs, uniques }: { movs: Movimiento[], uniques: Uniques }) {
  ...
  const { visibles, gtinInfo, saldoInicial, saldoFinal, totalEnt, totalSal } = useMemo(() => {
    ...
    const allGtin = movs.filter(r => r.gtin === gtinRef).sort((a, b) => (a.fechaHora?.getTime() || 0) - (b.fechaHora?.getTime() || 0));
    
    if (!allGtin.length) return { visibles: [], gtinInfo: null, saldoInicial: 0, saldoFinal: 0, totalEnt: 0, totalSal: 0 };

    const saldoPorAlmacen: Record<string, number> = {};
    let saldoConsol = 0;
    
    const assentamientos = allGtin.map(r => {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data), 'Trazabilidad_GTIN');
    XLSX.writeFile(wb, `Trazabilidad_${gtinRef}_${toISODate(new Date())}.xlsx`);
  };
  ...
  const handleExportPDF = () => {
    ...
    doc.text('Tarjeta de Trazabilidad por GTIN', 14, 30);
    
    doc.setFontSize(9);
    doc.setTextColor(100, 100, 106);
    doc.text(`Producto: ${gtinInfo?.nombre || ''}`, 14, 38);
    doc.text(`GTIN: ${gtinRef}  |  Código Ref: ${gtinInfo?.codRef || ''}  |  UPN: ${gtinInfo?.upn || ''}`, 14, 43);
    ...
    autoTable(doc, {
      startY: 58,
      head: [['F/H', 'Tipo', 'Almacén', 'Cód.Ref', 'Marca', 'OC', 'Factura', 'Lote/Serie', 'Caduc.', 'Usuario', 'Ent.', 'Sal.', 'Saldo']],
    ...
    doc.save(`Trazabilidad_${gtinRef}_${toISODate(new Date())}.pdf`);
  };

  const gtinOptions = uniques.gtins.map(g => ({
    value: g.gtin,
    label: `GTIN: ${g.gtin} · ${g.nombre} (${g.codRef})`
  }));
    ...
    <div>
      <div className="bg-white border border-[var(--color-line)] rounded-[14px] p-4 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
          <div className="flex flex-col gap-1 lg:col-span-2">
            <label className="text-[11px] text-[var(--color-ink-muted)] font-semibold uppercase tracking-wider">GTIN *</label>
            <Select 
              options={gtinOptions}
              value={gtinOptions.find(o => o.value === gtinRef) || null}
              onChange={(val) => setGtinRef(val?.value || '')}
      ...
      {!gtinRef ? (
        <div className="text-center py-12 px-5 text-[var(--color-ink-muted)]">
          <div className="text-5xl opacity-30 mb-2">🏷️</div>
          Selecciona un GTIN para ver su trazabilidad
        </div>
      ) : (
        <React.Fragment>
          {/* Header Stats */}
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-4 p-4 bg-[var(--color-red-pale)] rounded-[14px] border border-[#F5D4D9]">
            <div className="flex flex-col gap-0.5 col-span-2 md:col-span-4 lg:col-span-2">
              <span className="text-[10.5px] text-[var(--color-red-deep)] uppercase tracking-wider font-bold">GTIN Tracking</span>
              <span className="text-sm font-semibold text-[var(--color-ink)] truncate">{gtinRef}</span>
              <span className="font-mono text-[11px] text-[var(--color-ink-muted)] mt-0.5" title={gtinInfo?.nombre}>{gtinInfo?.nombre} ({gtinInfo?.codRef})</span>
            </div>
          ...
          <div className="bg-white border border-[var(--color-line)] rounded-[14px] overflow-hidden">
            ...
            {visibles.length > 0 && (
              <div className="flex items-center justify-between px-4 py-3 bg-[var(--color-surface)] border-t border-[var(--color-line)] text-[12.5px] text-[var(--color-ink-muted)]">
                ...
                <span>Última actualización: {toISODate(new Date())}</span>
              </div>
            )}
          </div>
        </React.Fragment>
